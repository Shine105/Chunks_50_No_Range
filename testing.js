  ...
  function addDataToOutput(rowValues, startRow) {
    rowValues.forEach(({ value, col }) => {
      ...
      columnData.forEach((dataValue, index) => {
        outputData.push(['HSN', nameOfStation, date, timeIntervals[index], value, dataValue]);
      });
    });
  }

  // Add the data from the third row and 1440 rows of SCADA tag data
  addDataToOutput(thirdRowValues, 6); // Starts from the 7th row (index 6)
}
...
fs.readdir(inputFolder, (err, files) => {
  ...
  for (let batchIndex = 0; batchIndex < batchCount; batchIndex++) {
    outputData = [['Zone', 'Name of Station', 'Date', 'Time', 'SCADA Tag', 'Data']]; // Reset outputData for each batch
